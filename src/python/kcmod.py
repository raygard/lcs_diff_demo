...
# Compute longest common subsequence (LCS) using my (rdg) modification of
# Kuo and Cross's modification of Hunt and Szymanski's method
#
...
def LCS(A, B):
    ...
    for i in range(1, m+1):
        ...
        for j in matchlist[i]:
            if j > temp:
                # My (rdg) modification: use binary search
                # in place of (not Python) "do k += 1 while j > thresh[k]"
                k = bisect.bisect_left(thresh, j, k)
